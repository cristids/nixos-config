...
with lib; let
  ...
  iiorientScript = handler: hpkg: devices: monitors: let
...
in {
  ...
  config = mkIf cfg.enable {
    systemd.user.services.iiorient = {
      Unit = {
        Description = "iiorient service: execute action based on iio-sensor-proxy's monitor-sensor output.";
        After = ["graphical-session.target"];
      };
      Install = {WantedBy = ["graphical-session.target"];};
      Service = {
        ExecStart =
          iiorientScript cfg.handler cfg.handlerPackage cfg.devices
          cfg.monitors;
      };
    };
  };
